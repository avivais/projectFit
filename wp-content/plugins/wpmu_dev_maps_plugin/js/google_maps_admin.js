...
AgmMapHandler = function (selector, data, allowInsertion) {
	...
	var saveMap = function () {
		...
		$(document).trigger('agm_google_maps-admin-save_request', [request]);
	...
	var createMarkup = function () {
		...
		$container.html(
			...
			'<div id="agm_mh_options" style="display:none">' +
				...
				'<fieldset><legend>' + l10nStrings.map_appearance + '</legend>' +
					'<div><input type="checkbox" id="agm_map_show_posts" /> <label for="agm_map_show_posts">' + l10nStrings.show_posts + '</label></div>' +
					'<div><input type="checkbox" id="agm_map_show_map" /> <label for="agm_map_show_map">' + l10nStrings.show_map + '</label></div>' +
					'<div><input type="checkbox" id="agm_map_show_markers" /> <label for="agm_map_show_markers">' + l10nStrings.show_markers + '</label></div>' +
					'<div><fieldset>' +
						'<legend>' + l10nStrings.images_strip + '</legend>' +
						'<input type="checkbox" id="agm_map_show_images" /> <label for="agm_map_show_images">' + l10nStrings.show_images + '</label>' +
						'<br />' +
					...
					'<fieldset><legend>' + l10nStrings.panoramio_overlay + '</legend>' +
						'<div><input type="checkbox" id="agm_map_show_panoramio_overlay" /> <label for="agm_map_show_panoramio_overlay">' + l10nStrings.show_panoramio_overlay + '</label></div>' +
						'<div><label for="agm_map_panoramio_overlay_tag">' + l10nStrings.panoramio_overlay_tag + '</label> <input type="text" size="12" id="agm_map_panoramio_overlay_tag" /></div>' +
		...
		$(document).trigger('agm_google_maps-admin-markup_created', [$container, data]);
		...
		// Fix lo-res screens map height
		if ($('#map_preview').height() < 320) {
			$("#TB_window").height($(window).height()-100).css('overflow-y', 'scroll');
			$("#TB_ajaxContent").width(550).css("margin", 0).css("padding", 10);
			$('#map_preview').height(320);
			$container.css({
				"height": "auto",
				"overflow-y": "none"
			});
		...
		$('#agm_mh_options').dialog({
			...
			"buttons": { 
				"OK": function() { 
					$(this).dialog("close"); 
					$(document).trigger('agm_google_maps-admin-options_dialog-closed', [map]);
				} 
			}
		});
		...
		$(document).trigger('agm_google_maps-admin-options_initialized', [$container, data]);
	};
	...
	var init = function () {
		...
		$(document).trigger('agm_google_maps-admin-map_initialized', [map, data]);
