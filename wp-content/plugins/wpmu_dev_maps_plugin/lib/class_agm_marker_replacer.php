...
class AgmMarkerReplacer {
	...
	function process_tags ($atts, $content=null) {
		$body = false;
		$atts = shortcode_atts(apply_filters('agm_google_map-shortcode-attributes_defaults', array(
			'id' => false,
			'query' => false,
			'overlay' => false,
			'network' => false,
...
$atts['query'] = preg_replace(
	...
	preg_replace('/' . preg_quote('&amp;') . '/', '&', $atts['query'])
		...
		$overrides = apply_filters('agm_google_map-shortcode-overrides_process', $overrides, $atts);
